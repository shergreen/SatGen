...
z0 = 0.6*hDekel.rh
VR0 = 0.
Vphi0 = Vcirc(potential,np.sqrt(R0**2+z0**2),0.)*0.3
Vz0 = 0.
#
...
ttdyn0_grid = np.array([0.,0.85,0.95,1.05,1.15,1.25,1.35])
...
fig1.subplots_adjust(left=0.065, right=0.95,bottom=0.06, top=0.99,
...
if (Rmax >= 100.):
    ...
    major_ticks = np.arange(start, end, 20.)
...
# annotations
ax.text(lv0,ax.get_ylim()[0]*2,r'$l_\mathrm{v}$',color='k',fontsize=16,
...
ax.plot(np.repeat(lv0,2),ax.get_ylim(),ls=':',color='k')
# annotations
ax.text(lv0,ax.get_ylim()[1]*0.1,r'$l_\mathrm{v}$',color='k',fontsize=16,
    ha='left',va='bottom',transform=ax.transData,rotation=90)
# legends
...
ax.legend(loc='best',numpoints=1,scatterpoints=1,fontsize=14,
    frameon=True) #bbox_to_anchor=(1.2, 0.9)
# annotation
ax.text(0.95*tmax,0.1*lv0,r'$0.1l_\mathrm{v}(0)$',color='k',
